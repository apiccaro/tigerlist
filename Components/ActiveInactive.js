...
import Link from 'next/link';
  ...
  * 
  * JM - I don't think this is how it's currently supposed to be done 
  */
...
const ActiveInactive = ({post_key, title, price, description, category, condition, location, email, phone, images, flagged, active}) => {
  ...
  return (
    <div>
        <EditLink post_key={post_key} title={title} price={price} description={description} category={category} condition={condition} location={location} email={email} phone={phone} images={images} flagged={flagged}/>
        <button
        style={{ width: '80px', height: '30px', display: 'flex',  fontWeight: '600', alignItems: 'center', justifyContent: 'center', borderRadius: '10px', fontSize:"1.3rem", backgroundColor: isActive ? 'white' : 'black', color: isActive ? 'black' : 'white'}}
           // onClick={toggleButtonText} className="flex font-bold text-xl"> {isActive ? 'Active' : 'Hold'} //Version used in Jay-B6
          onClick={() => handleClick({post_key, title, price, description, category, condition, location, email, phone, images, active, flagged, banned})}        >
