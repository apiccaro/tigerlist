    ...
    return(
        <div className="flex flex-col gap-1 text-4xl font-semibold">
            ...
            {myListings.map((listing, index) => (
                <SellProductBox key={index} title={listing.title} price={listing.price} description={listing.description} category={listing.category} condition={listing.condition} location={listing.location} email={listing.email} phone={listing.phone} images={listing.images}/>
            ))}
            </ProductsGrid>
        </div>
    );
